<x-app-layout title="Edit Meal Plan" is-header-blur="true">
    <main class="main-content w-full px-[var(--margin-x)] pb-8">
        <!-- Breadcrumb -->
        <div class="flex items-center space-x-4 py-5 lg:py-6">
            <h2 class="text-xl font-medium text-slate-800 dark:text-navy-50 lg:text-2xl">Edit Meal Plan</h2>
            ...
            <div class="col-span-12 lg:col-span-8">
                <div class="card">
                    <div class="flex items-center justify-between border-b border-slate-200 px-4 py-4 dark:border-navy-500 sm:px-5">
                        <h2 class="text-lg font-medium text-slate-700 dark:text-navy-100">Edit Meal Plan Information</h2>
                        <a href="{{ route('dietitian.meal-plans.index') }}"
                    ...
                    <div class="p-4 sm:p-5" x-data="mealPlanBuilder()">
                        <form method="POST" action="{{ route('dietitian.meal-plans.update', $mealPlan->id) }}">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <label class="block">
                                    ...
                                    <input name="name" type="text" value="{{ old('name', $mealPlan->name) }}"
                                ...
                                <label class="block">
                                    ...
                                    <select name="patient_id" required
                                        ...
                                        @foreach ($patients as $patient)
                                            <option value="{{ $patient->id }}" {{ $mealPlan->patient_id == $patient->id ? 'selected' : '' }}>
                                                {{ $patient->user->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <label class="block mt-4">
                                ...
                                <textarea name="notes" rows="3"
                                    class="form-textarea mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 dark:border-navy-450 dark:bg-navy-700">{{ old('notes', $mealPlan->notes) }}</textarea>
                            </label>
                                        ...
                                        <div class="p-4 space-y-4">
                                            <template x-for="(meal, mealIndex) in mealType.meals" :key="mealIndex">
                                                <div class="rounded-lg border border-slate-200 dark:border-navy-600 bg-white dark:bg-navy-700 p-4">
                                                    ...
                                                    <!-- Hidden inputs for meal ID and type -->
                                                    <input type="hidden" :name="'meals[' + getMealIndex(typeIndex, mealIndex) + '][id]'" x-model="meal.id">
                                                    ...
                                                    <div class="mt-4">
                                                        ...
                                                        <template x-for="(item, itemIndex) in meal.items" :key="itemIndex">
                                                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3 items-end mt-3 p-3 rounded-lg bg-slate-50 dark:bg-navy-600 border border-slate-200 dark:border-navy-500">
                                                                ...
                                                                <input type="hidden" :name="'meals[' + getMealIndex(typeIndex, mealIndex) + '][items][' + itemIndex + '][id]'" x-model="item.id">
                                                                ...
                                                                <label class="block">
                                                                    ...
                                                                    <select x-model="item.food_item_id" :name="'meals[' + getMealIndex(typeIndex, mealIndex) + '][items][' + itemIndex + '][food_item_id]'"
                                                                        class="form-select mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 dark:border-navy-450 dark:bg-navy-700" required>
            ...
            <div class="col-span-12 lg:col-span-4">
                <div class="card">
                    ...
                    <div class="p-4 sm:p-5 text-sm text-slate-600 dark:text-navy-300 space-y-2">
                        <p><strong class="font-medium">Meals are organized by type</strong> for better structure and clarity.</p>
                        <p>You can add multiple meal options within each category for variety.</p>
                        <p>Ensure each portion is realistic and easily measurable.</p>
                        <p>Use the notes field to add preparation instructions or dietary considerations.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        function mealPlanBuilder() {
            return {
                ...
                init() {
                    ...
                    this.loadExistingMeals(@json($mealData));
                },

                loadExistingMeals(existingMeals) {
                    // Group existing meals by type
                    existingMeals.forEach(meal => {
                        const mealTypeIndex = this.mealTypes.findIndex(type => type.type === meal.type);
                        if (mealTypeIndex !== -1) {
                            this.mealTypes[mealTypeIndex].meals.push(meal);
                        } else {
                            // If meal type doesn't exist in predefined types, add to breakfast
                            this.mealTypes[0].meals.push({
                                ...meal,
                                type: 'breakfast'
                            });
                        }
