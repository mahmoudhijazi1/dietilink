    ...
    <main class="main-content w-full px-[var(--margin-x)] pb-8">
        ...
        <div class="grid grid-cols-12 gap-4 sm:gap-5 lg:gap-6">
            <div class="col-span-12 lg:col-span-8">
                <div class="card" x-data="progressEntryForm()">
                        ...
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-slate-500 dark:text-navy-300">
                                    Record {{ $patient->user->name }}'s progress measurements
                                </p>
                            </div>
                        </div>
                    ...
                    <div class="p-4 sm:p-5">
                        ...
                        <form method="POST" action="{{ route('dietitian.progress.store', $patient->id) }}" 
                            ...
                            <div class="space-y-6">
                                <div class="flex items-center justify-between">
                                    ...
                                    <button type="button" 
                                        ...
                                        <span x-text="showMeasurements ? 'Hide Measurements' : 'Add Measurements'"></span>
                            ...
                            <div class="space-y-6">
                                <div class="flex items-center justify-between">
                                    ...
                                    <button type="button" 
                                            ...
                                            class="btn rounded-lg bg-slate-100 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-200 dark:bg-navy-600 dark:text-navy-100 dark:hover:bg-navy-500 transition-all duration-200">
                                        <span x-text="showComposition ? 'Hide Composition' : 'Add Composition'"></span>
                                        <svg xmlns="http://www.w3.org/2000/svg" class="size-4 ml-2 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-slate-500 dark:text-navy-300">Upload up to 5 progress photos (max 5MB each)</p>
                                        </div>
                                    </div>
                                    <button type="button" 
                                            @click="showImages = !showImages" 
                                            class="btn rounded-lg bg-slate-100 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-200 dark:bg-navy-600 dark:text-navy-100 dark:hover:bg-navy-500 transition-all duration-200">
                                        <span x-text="showImages ? 'Hide Images' : 'Add Images'"></span>
                                        <svg xmlns="http://www.w3.org/2000/svg" class="size-4 ml-2 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                                    ...
                                    <div class="border-2 border-dashed border-slate-300 rounded-lg p-6 dark:border-navy-600">
                                        <div class="text-center">
                                            ...
                                            <p class="text-xs text-slate-500 dark:text-navy-300">
                                                JPEG, PNG, JPG or WEBP up to 5MB each (max 5 images)
                                            </p>
                                        </div>
                            ...
                            <div class="space-y-4">
                                ...
                                <label class="block">
                                    ...
                                    <textarea
                                        ...
                                        rows="4" 
                                        name="notes" 
                                        placeholder="Add any notes about this progress entry (diet changes, exercise routine, mood, energy levels, etc.)">{{ old('notes') }}</textarea>
                                </label>
                            </div>
            ...
            <div class="col-span-12 lg:col-span-4">
                <div class="card p-4 sm:p-5">
                    ...
                    <div class="mt-5 space-y-4">
                        ...
                        <!-- Latest Progress -->
                        <div>
                            <h4 class="font-medium text-slate-600 dark:text-navy-100 mb-3">Latest Progress</h4>
                            @php
                                $latestProgress = \App\Models\ProgressEntry::where('patient_id', $patient->id)
                            ...
                            @if($latestProgress)
                                <div class="space-y-2 text-sm">
                                    ...
                                    <div class="flex justify-between">
                                        ...
                                        <div class="font-medium">{{ $latestProgress->weight }} kg</div>
                                    </div>
                                    @if($latestProgress->chest || $latestProgress->waist || $latestProgress->hips)
                                        <div class="pt-2 border-t border-slate-200 dark:border-navy-500">
                                            ...
                                            @if($latestProgress->chest)
                                                <div class="flex justify-between text-xs">
                                                    <span>Chest:</span><span>{{ $latestProgress->chest }} cm</span>
                                                </div>
                                            @endif
                                            @if($latestProgress->waist)
                                                <div class="flex justify-between text-xs">
                                                    <span>Waist:</span><span>{{ $latestProgress->waist }} cm</span>
                                                </div>
                                            @endif
                                            @if($latestProgress->hips)
                                                <div class="flex justify-between text-xs">
                                                    <span>Hips:</span><span>{{ $latestProgress->hips }} cm</span>
                                                </div>
                                            @endif
                                        </div>
                                    @endif
                                    @if($latestProgress->progressImages->count() > 0)
                                        <div class="pt-2 border-t border-slate-200 dark:border-navy-500">
                                            <div class="text-xs text-slate-400 dark:text-navy-400 mb-1">
                                                {{ $latestProgress->progressImages->count() }} progress image{{ $latestProgress->progressImages->count() > 1 ? 's' : '' }}
                                            </div>
                                        </div>
                                    @endif
                                </div>
                            @else
                                <div class="text-center py-4">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="size-12 mx-auto text-slate-300 dark:text-navy-600 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/>
                                    </svg>
                                    <p class="text-slate-500 dark:text-navy-300 text-sm">No progress entries yet</p>
                                    <p class="text-slate-400 dark:text-navy-400 text-xs mt-1">This will be the first one!</p>
                                </div>
                            @endif
                        </div>
                    </div>
