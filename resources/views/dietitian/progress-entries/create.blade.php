<x-app-layout title="Add Progress Entry" is-header-blur="true">
                ...
                <div class="card">
                    <div class="flex flex-col items-center space-y-4 border-b border-slate-200 p-4 dark:border-navy-500 sm:flex-row sm:justify-between sm:space-y-0 sm:px-5">
                        <h2 class="text-lg font-medium tracking-wide text-slate-700 dark:text-navy-100">
                            Add New Progress Entry for {{ $patient->user->name }}
                        </h2>
                    ...
                    <div class="p-4 sm:p-5">
                        ...
                        <form method="POST" action="{{ route('dietitian.progress.store', $patient->id) }}" class="mt-4 space-y-5">
                            @csrf
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <label class="block">
                                    ...
                                    <input
                                        ...
                                        placeholder="Weight in kilograms" type="number" step="0.1" min="0" name="weight" value="{{ old('weight') }}" required />
                                </label>
                                ...
                                <label class="block">
                                    ...
                                    <input
                                        class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                        type="date" name="measurement_date" value="{{ old('measurement_date', date('Y-m-d')) }}" required />
                                </label>
                            </div>

                            <!-- Optional Measurements -->
                            <div x-data="{ showMeasurements: false }">
                                ...
                                <div x-show="showMeasurements" class="grid grid-cols-1 gap-4 sm:grid-cols-3 mt-4" style="display: none;">
                                    <label class="block">
                                        <span class="font-medium text-slate-600 dark:text-navy-100">Chest (cm)</span>
                                        <input
                                            class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                            placeholder="Chest measurement" type="number" step="0.1" min="0" name="measurements[chest]" value="{{ old('measurements.chest') }}" />
                                    </label>
                                    ...
                                    <label class="block">
                                        ...
                                        <input
                                            class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                            placeholder="Waist measurement" type="number" step="0.1" min="0" name="measurements[waist]" value="{{ old('measurements.waist') }}" />
                                    </label>
                                    ...
                                    <label class="block">
                                        ...
                                        <input
                                            class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                            placeholder="Hips measurement" type="number" step="0.1" min="0" name="measurements[hips]" value="{{ old('measurements.hips') }}" />
                                    </label>
                                    ...
                                    <label class="block">
                                        ...
                                        <input
                                            class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                            placeholder="Arms measurement" type="number" step="0.1" min="0" name="measurements[arms]" value="{{ old('measurements.arms') }}" />
                                    </label>
                                    ...
                                    <label class="block">
                                        ...
                                        <input
                                            class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                            placeholder="Thighs measurement" type="number" step="0.1" min="0" name="measurements[thighs]" value="{{ old('measurements.thighs') }}" />
                                    </label>
                                    ...
                                    <label class="block">
                                        ...
                                        <input
                                            class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                            placeholder="Body fat percentage" type="number" step="0.1" min="0" max="100" name="measurements[body_fat]" value="{{ old('measurements.body_fat') }}" />
                                    </label>
                                </div>
                            </div>
                            ...
                            <label class="block">
                                ...
                                <textarea
                                    ...
                                    rows="4" name="notes" placeholder="Notes about this progress entry">{{ old('notes') }}</textarea>
                            </label>
            ...
            <div class="col-span-12 lg:col-span-4">
                <div class="card p-4 sm:p-5">
                    ...
                    <div class="mt-5 space-y-4">
                        ...
                        <div>
                            <h3 class="font-medium text-slate-600 dark:text-navy-100 mb-2">Latest Progress</h3>
                            @php
                                $latestProgress = \App\Models\ProgressEntry::where('patient_id', $patient->id)->latest('measurement_date')->first();
                            @endphp

                            @if($latestProgress)
                                <div class="space-y-2">
                                    <div class="flex justify-between">
                                        <div class="text-xs+ text-slate-400 dark:text-navy-300">Date</div>
                                        <div>{{ $latestProgress->measurement_date->format('M d, Y') }}</div>
                                    </div>
                                    <div class="flex justify-between">
                                        <div class="text-xs+ text-slate-400 dark:text-navy-300">Weight</div>
                                        <div>{{ $latestProgress->weight }} kg</div>
                                    </div>
                                    @if(!empty($latestProgress->measurements))
                                        <div class="flex justify-between">
                                            <div class="text-xs+ text-slate-400 dark:text-navy-300">Measurements</div>
                                            <div>
                                                @foreach($latestProgress->measurements as $key => $value)
                                                    <div class="text-right">{{ ucfirst($key) }}: {{ $value }} {{ $key === 'body_fat' ? '%' : 'cm' }}</div>
                                                @endforeach
                                            </div>
                                        </div>
                                    @endif
                            ...
                            @else
                                <p class="text-slate-500 dark:text-navy-300 italic">No progress entries recorded yet</p>
                            @endif
                        </div>
                    </div>
                </div>
